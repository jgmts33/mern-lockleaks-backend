...
export const downloadDataReport = async (info) => {
    ...
    doc.save(`${homeDirectory}/lockleaks-backend/pdfs/data-report_from_${moment(new Date().setMonth(new Date().getMonth() - 1)).format("DD_MMM_YYYY")}_to_${moment(new Date()).format("DD_MMM_YYYY")}_${info.user_id}.pdf`);
